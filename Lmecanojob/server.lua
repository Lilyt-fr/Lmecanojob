...
ESX.RegisterServerCallback('mecano:getStockItems', function(source, cb)
	TriggerEvent('esx_addoninventory:getSharedInventory', societyName, function(inventory)
		cb(inventory.items)
	end)
end)

RegisterNetEvent('mecano:getStockItem')
AddEventHandler('mecano:getStockItem', function(itemName, count)
	local _src = source
...
ESX.RegisterServerCallback('mecano:getPlayerInventory', function(source, cb)
    local _src = source
	local xPlayer = ESX.GetPlayerFromId(_src)
...
RegisterNetEvent('mecano:putStockItems')
AddEventHandler('mecano:putStockItems', function(itemName, count)
	local _src = source
	local xPlayer = ESX.GetPlayerFromId(_src)
...
RegisterServerEvent('mecano:onNPCJobMissionCompleted')
AddEventHandler('mecano:onNPCJobMissionCompleted', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent("mecano:retraitentreprise")
AddEventHandler("mecano:retraitentreprise", function(money)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local total = money
...
RegisterServerEvent("mecano:depotentreprise")
AddEventHandler("mecano:depotentreprise", function(money)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local total = money
    local xMoney = xPlayer.getMoney()
...
RegisterServerEvent('mecano:recruter')
AddEventHandler('mecano:recruter', function(target)
...
RegisterServerEvent('mecano:promouvoir')
AddEventHandler('mecano:promouvoir', function(target)
...
RegisterServerEvent('mecano:descendre')
AddEventHandler('mecano:descendre', function(target)

  local xPlayer = ESX.GetPlayerFromId(source)
  local xTarget = ESX.GetPlayerFromId(target)

  	if xPlayer.job.grade_name == 'boss' and xPlayer.job.name == xTarget.job.name then
  	xTarget.setJob("mecano", tonumber(xTarget.job.grade) - 1)
...
RegisterServerEvent('mecano:virer')
AddEventHandler('mecano:virer', function(target)
