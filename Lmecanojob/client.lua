...
Citizen.CreateThread(function()
    ...
    AddTextComponentString("Mécano - Nord")
                            ...
                            if (GetOnscreenKeyboardResult()) then
                                ...
                                if result then
                                    ...
                                    if player ~= -1 and distance <= 3.0 then
                                        TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_mecano', ("Benny's"), montant)
						...
						if NPCOnJob then
							if NPCTargetDeleterZone then
								...
								if CurrentlyTowedVehicle == NPCTargetTowable then
									...
									TriggerServerEvent('mecano:onNPCJobMissionCompleted')
									StopNPCJob()
    ...
    while true do
        ...
        local dist = #(plyPos-Config.boutiquekit)
            if ESX.PlayerData.job and ESX.PlayerData.job.name == 'mecano' then 
                if dist <= 15.0 then
                wait = 0
				DrawMarker(20,  Config.boutiquekit.x, Config.boutiquekit.y, Config.boutiquekit.z, nil, nil, nil, 0, nil, nil, 0.3, 0.3, 0.3, 230, 230, 0 , 120)
                DrawMarker(1,  Config.boutiquekit.x, Config.boutiquekit.y, Config.boutiquekit.z-0.99, nil, nil, nil, 0, nil, nil, 1.0, 1.0, 0.2, 255, 255, 255 , 255)
                end
            if dist <= 1.0 then
                wait = 0
...
Citizen.CreateThread(function()
	while true do
		...
		local dist = #(plyPos-Config.garage)
			if dist <= 20.0 then
				wait = 0
				DrawMarker(20,  Config.garage.x, Config.garage.y, Config.garage.z, nil, nil, nil, 0, nil, nil, 0.3, 0.3, 0.3, 230, 230, 0 , 120)
				DrawMarker(1,  Config.garage.x, Config.garage.y, Config.garage.z-0.99, nil, nil, nil, 0, nil, nil, 1.0, 1.0, 0.2, 255, 255, 255 , 255)
			end
            if dist <= 1.0 then
                wait = 0
            if ESX.PlayerData.job and ESX.PlayerData.job.name == 'mecano' or ESX.PlayerData.job2 and ESX.PlayerData.job2.name == 'mecano' then    
...
Citizen.CreateThread(function()
    while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'mecano' then 
			if dist <= 50.0 then
			wait = 0
			DrawMarker(20,  Config.vestiaire.x, Config.vestiaire.y, Config.vestiaire.z, nil, nil, nil, 0, nil, nil, 0.3, 0.3, 0.3, 230, 230, 0 , 120)
			DrawMarker(1,  Config.vestiaire.x, Config.vestiaire.y, Config.vestiaire.z-0.99, nil, nil, nil, 0, nil, nil, 1.0, 1.0, 0.2, 255, 255, 255 , 255)
			end
			if dist <= 1.0 then
				wait = 0
...
Citizen.CreateThread(function()
    while true do
        ...
        if dist <= 50.0 then
         Timer = 0
         DrawMarker(20,  Config.coffre.x, Config.coffre.y, Config.coffre.z, nil, nil, nil, 0, nil, nil, 0.3, 0.3, 0.3, 230, 230, 0 , 120)
		 DrawMarker(1,  Config.coffre.x, Config.coffre.y, Config.coffre.z-0.99, nil, nil, nil, 0, nil, nil, 1.0, 1.0, 0.2, 255, 255, 255 , 255)
        end
         if dist <= 1.0 then
            Timer = 0
...
function menuCoffreRetirer()
    ...
    ESX.TriggerServerCallback('mecano:getStockItems', function(items) 
    itemstock = items
                        ...
                        for k,v in pairs(itemstock) do 
                            ...
                            RageUI.ButtonWithStyle(v.label, nil, {RightLabel = v.count}, true, function(Hovered, Active, Selected)
                                if Selected then
                                    ...
                                    TriggerServerEvent('mecano:getStockItem', v.name, tonumber(count))
                                    RageUI.CloseAll()
                                end
                            end)
                        end
...
function menuCoffreDeposer()
    ...
    ESX.TriggerServerCallback('mecano:getPlayerInventory', function(inventory)
                         ...
                         local item = inventory.items[i]
                            if item.count > 0 then
                                    RageUI.ButtonWithStyle(item.label, nil, {RightLabel = item.count}, true, function(Hovered, Active, Selected)
                                            ...
                                            TriggerServerEvent('mecano:putStockItems', item.name, tonumber(count))
                                            RageUI.CloseAll()
                                        end
                                    end)
                                end
                ...
                if Selected then
					...
					if amount ~= nil then
						...
						if type(amount) == 'number' then
							TriggerServerEvent('mecano:retraitentreprise', amount)
						else
            ...
            RageUI.ButtonWithStyle("Déposer de l'argent",nil, {RightLabel = "→"}, true, function(Hovered, Active, Selected)
                if Selected then
                    ...
                    if amount == nil then
						...
						TriggerServerEvent('mecano:depotentreprise', amount)
                    end
                end
            end)
			...
			RageUI.ButtonWithStyle("Recruter", nil, {RightLabel = "→"},not cooldown, function(Hovered, Active, Selected)
				if (Selected) then   
					...
					if closestPlayer ~= -1 and closestDistance <= 3.0 then
						TriggerServerEvent('mecano:recruter', GetPlayerServerId(closestPlayer))
					 else
				...
				if (Selected) then   
					local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
					if closestPlayer ~= -1 and closestDistance <= 3.0 then
						TriggerServerEvent('mecano:promouvoir', GetPlayerServerId(closestPlayer))
					 else
						RageUI.Popup({message = "Aucun joueur à proximité"})
					end 
					cooldowncool(1000)
				...
				if (Selected) then   
					local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
					if closestPlayer ~= -1 and closestDistance <= 3.0 then
						TriggerServerEvent('mecano:descendre', GetPlayerServerId(closestPlayer))
					 else
						RageUI.Popup({message = "Aucun joueur à proximité"})
					end 
					cooldowncool(1000)
				...
				if (Selected) then   
					local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
					if closestPlayer ~= -1 and closestDistance <= 3.0 then
						TriggerServerEvent('mecano:virer', GetPlayerServerId(closestPlayer))
					 else
						RageUI.Popup({message = "Aucun joueur à proximité"})
					end 
					cooldowncool(1000)
...
Citizen.CreateThread(function()
    while true do
        ...
        if dist <= 50.0 then
         Timer = 0
		 DrawMarker(20, Config.Boss.x, Config.Boss.y, Config.Boss.z, nil, nil, nil, 0, nil, nil, 0.3, 0.3, 0.3, 230, 230, 0 , 120)
		 DrawMarker(1, Config.Boss.x, Config.Boss.y, Config.Boss.z-0.99, nil, nil, nil, 0, nil, nil, 1.0, 1.0, 0.2, 255, 255, 255 , 255)
        end
         if dist <= 1.0 then
            Timer = 0
